(function(w) {
    ...
    w.shareText = function(score) {
        ...
        if (deviation_time > 23000) {
            return '啊呜，开车时间是不是太久了(。G ∩ G)' + ((deviation_time / 1000) - 20).toFixed(2) + "s";
        }
        SubmitResults();
        if (score <= 49) return '不合格(。G ∩ G)';
        if (score <= 99) return 'bad bad 2 me OH YEAH!';
        if (score <= 149) return '合格(。G ∪ G)';
